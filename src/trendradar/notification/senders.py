...
import requests
from trendradar.core.constants import BATCH, TIMEOUT, RETRY
from trendradar.utils.image import is_valid_image_url, get_default_banner
from trendradar.utils.retry import retry_on_exception

from .batch import add_batch_headers, get_max_batch_header_size
from trendradar.utils.text import convert_markdown_to_mrkdwn, strip_markdown
...
class SendConfig:
    ...
    batch_interval: float = BATCH.DEFAULT_BATCH_INTERVAL
...
class ChannelSender(ABC):
    ...
    def __init__(self, config: SendConfig):
        self.config = config
        self.log_prefix = self._get_log_prefix()
    ...
    def _prepare_batches(self) -> list:
        """准备批次"""
        header_reserve = get_max_batch_header_size(self._get_channel_name())
        batches = self.config.split_content_func(
            self.config.report_data,
            self._get_channel_name(),
            self.config.update_info,
            max_bytes=self.config.batch_size - header_reserve,
            mode=self.config.mode,
            rss_items=self.config.rss_items,
            rss_new_items=self.config.rss_new_items,
        )
        return add_batch_headers(batches, self._get_channel_name(), self.config.batch_size)
    ...
    @retry_on_exception(
        ...
        base_wait=RETRY.BASE_WAIT_SECONDS,
        max_wait=RETRY.MAX_WAIT_SECONDS,
        exponential=True
    )
...
class FeishuSender(ChannelSender):
    ...
    def _get_log_prefix(self) -> str:
        return f"飞书{self.config.account_label}" if self.config.account_label else "飞书"
    
    def _get_channel_name(self) -> str:
        return "feishu"
    
    def _build_payload(self, content: str) -> Dict:
        now = self.config.get_time_func() if self.config.get_time_func else datetime.now()
        total_titles = sum(
            len(stat["titles"]) for stat in self.config.report_data["stats"] if stat["count"] > 0
        )
...
def send_to_dingtalk(
    ...
    *,
    ...
    for i, batch_content in enumerate(batches, 1):
        ...
        # 获取自定义标题
        report_title_base = "TrendRadar 热点分析"
                    ...
                    for group in report_data["stats"]:
                        if isinstance(group, dict) and "titles" in group:
                            for item in group["titles"]:
                                t = item.get("title")
                                u = item.get("url")
                                # 优先使用 display_image_url
                                img_url = item.get("display_image_url") or item.get("image_url")
                                if t and u and t not in seen_titles:
                                    top_items.append({
                                        "title": t,
                                        "url": u,
                                        "picurl": img_url if is_valid_image_url(img_url) else get_default_banner()
                                    })
                                    seen_titles.add(t)
                ...
                if len(top_items) < 5 and "new_titles" in report_data and isinstance(report_data["new_titles"], dict):
                    for source_id, titles_data in report_data["new_titles"].items():
                        for t, info in titles_data.items():
                            u = info.get("url")
                            # 优先使用 display_image_url (本地缓存的 Web 直接链接)
                            img_url = info.get("display_image_url") or info.get("image_url")
                            if t and u and t not in seen_titles:
                                top_items.append({
                                    "title": t,
                                    "url": u,
