...
def _load_app_config(config_data: Dict) -> Dict:
    ...
    return {
        ...
        "WEB_URL": _get_env_str("WEB_URL") or app_config.get("web_url", ""),
    }
...
def _load_notification_config(config_data: Dict) -> Dict:
    ...
    return {
        ...
        # WEB_URL 现在统一在 _load_app_config 中处理，此处保留兼容性或留空
    }
...
def _load_rss_config(config_data: Dict) -> Dict:
    ...
    # RSS 代理配置：环境优先 -> RSS 专属代理 -> crawler 默认代理
    rss_proxy_url = _get_env_str("RSS_PROXY_URL") or advanced_rss.get("proxy_url", "") or advanced_crawler.get("default_proxy", "")
    rss_use_proxy = _get_env_bool("RSS_USE_PROXY")
    ...
    try:
        ...
        if max_age_days < 0:
            logger.warning(f"RSS freshness_filter.max_age_days 为负数 ({max_age_days})，使用默认值 3")
            max_age_days = 3
    except (ValueError, TypeError):
        logger.warning(f"RSS freshness_filter.max_age_days 格式错误 ({raw_max_age})，使用默认值 3")
        max_age_days = 3
    ...
    return {
        ...
        "USE_PROXY": rss_use_proxy if rss_use_proxy is not None else advanced_rss.get("use_proxy", False),
...
def load_config(config_path: Optional[str] = None) -> Dict[str, Any]:
    ...
    # 检查敏感信息
    _check_sensitive_info(config_path)
...
def _load_llm_config(config_data: Dict) -> Dict:
    ...
    enabled_env = _get_env_bool("LLM_ENABLED")

    return {
        "ENABLED": enabled_env if enabled_env is not None else llm.get("enabled", False),
        "PROVIDER": _get_env_str("LLM_PROVIDER") or llm.get("provider", "ollama"),
        "BASE_URL": _get_env_str("LLM_BASE_URL") or llm.get("base_url", "http://localhost:11434"),
        "API_KEY": _get_env_str("LLM_API_KEY") or llm.get("api_key", ""),
        "MODEL": _get_env_str("LLM_MODEL") or llm.get("model", "qwen2.5:7b"),
        "SCORE_THRESHOLD": llm.get("score_threshold", 6.0),
