...
class StorageManager:
    ...
    def get_image_cache(self) -> "ImageCache":
        ...
        if self._image_cache is None:
            try:
                ...
                # 图片缓存目录 output/cache/images
                cache_dir = os.path.join(self.data_dir, "cache", "images")
                
                self._image_cache = ImageCache(
                    cache_dir=cache_dir,
                    retention_days=self.local_retention_days or 30,
    ...
    def analytics_engine(self) -> "AnalyticsEngine":
        """获取 DuckDB 分析引擎"""
        return self.get_analytics_engine()
    ...
    def is_news_pushed_batch(self, hash_list: List[str]) -> Dict[str, bool]:
        """批量检查新闻是否已推送"""
        return self.get_backend().is_news_pushed_batch(hash_list)
