...
class AsyncRSSFetcher:
    ...
    async def fetch_feed(self, client: httpx.AsyncClient, feed: RSSFeedConfig) -> Tuple[List[RSSItem], Optional[str]]:
        """
        抓取单个 RSS 源 (异步)
    ...
    async def fetch_all(self) -> RSSData:
        ...
        for i, (items, error) in enumerate(results):
            feed = self.feeds[i]
    ...
    @classmethod
    def from_config(cls, config: Dict) -> "AsyncRSSFetcher":
        """
        从配置字典创建抓取器
        """
        # 读取新鲜度过滤配置
        ...
        for feed_config in config.get("feeds", []):
            # ... (Same config parsing logic as before) ...
