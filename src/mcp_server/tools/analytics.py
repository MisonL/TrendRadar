...
class AnalyticsTools:
    ...
    def __init__(self, project_root: Optional[str] = None):
        ...
        try:
            ...
            self.analytics = AnalyticsEngine(data_dir=data_dir)
        except Exception as e:
            print(f"[Analytics] DuckDB 引擎初始化失败: {e}")
            self.analytics = None
    ...
    def get_topic_trend_analysis(
        ...
        try:
            ...
            if self.analytics and granularity == "day":
                try:
                    # 同步数据（计算日期范围内的天数）
                    days_diff = (end_date - start_date).days + 1
                    self.analytics.sync_data(days=days_diff)
                    
                    conn = self.analytics.connect()
                    # 查询每日热度
                    # 注意：DuckDB 的 LIKE 是大小写敏感的，ILIKE 不区分大小写
                    query = """
                        SELECT 
                            partition_date, 
                    ...
                    date_map = {}
                    for row in result:
                        p_date, count, titles = row
                        # titles 是 list，可能包含重复，取前3个不重复的
                    ...
                    while current_date <= end_date:
                        ...
                        data = date_map.get(date_str, {"count": 0, "sample_titles": []})
                        trend_data.append({
                            "date": date_str,
                            "count": data["count"],
                    ...
                    used_duckdb = True
                    print(f"[Analytics] 使用 DuckDB 优化查询: {topic}")
                    
                except Exception as e:
                    print(f"[Analytics] DuckDB 查询失败，回退到文件扫描: {e}")
                    used_duckdb = False

            # 回退到文件扫描 (如果 DuckDB 不可用或失败)
            if not used_duckdb:
                current_date = start_date
                while current_date <= end_date:
                    try:
