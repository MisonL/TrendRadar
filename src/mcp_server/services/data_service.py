...
class DataService:
    ...
    def __init__(self, project_root: str = None):
        ...
        # 初始化图片缓存
        cache_dir = os.path.join(self.parser.project_root, "output", "cache", "images")
        self.image_cache = ImageCache(cache_dir=cache_dir)
        
        # 确定基础 URL (默认为本机 IP + 8000 端口)
        # 实际部署时应读取配置文件中的 WEB_URL
        self.base_url = f"http://{get_local_ip()}:8000/images"

    def _resolve_image_url(self, original_url: str) -> str:
        """将原始图片 URL 转换为本地缓存 URL (如果存在)"""
        if not original_url:
            return ""
            
        cached_url = self.image_cache.get_cached_url(original_url, self.base_url)
        return cached_url if cached_url else original_url
    ...
    def get_latest_news(
        ...
        for platform_id, titles in all_titles.items():
            ...
            for title, info in titles.items():
                ...
                # 处理图片 URL (总是包含，因为对前端展示很重要)
                if info.get("image_url"):
                    news_item["image_url"] = self._resolve_image_url(info.get("image_url"))
                ...
                for platform_id, titles in all_titles.items():
                    ...
                    for title, info in titles.items():
                        if keyword.lower() in title.lower():
                            ...
                            # 处理图片 URL
                            if info.get("image_url"):
                                results[-1]["image_url"] = self._resolve_image_url(info.get("image_url"))
    ...
    def get_latest_rss(
        ...
        for feed_id, items in all_items.items():
            ...
            for title, info in items.items():
                ...
                # 处理图片 URL
                if info.get("image_url"):
                    rss_item["image_url"] = self._resolve_image_url(info.get("image_url"))
